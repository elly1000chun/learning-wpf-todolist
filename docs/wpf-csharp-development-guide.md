...
마감일 입력 UI 단계에서는 이미 모델에 있던 `DueDate` 값을 실제 화면 입력과 편집 흐름에 연결했습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. 새 할 일 입력 상태로 `NewTodoDueDate`를 추가한다.
2. 새 할 일을 만들 때 `DueDate = NewTodoDueDate`로 선택한 날짜를 저장한다.
3. 할 일 추가가 끝나면 `NewTodoDueDate = null`로 날짜 입력 상태를 초기화한다.
4. `MainWindow.xaml`의 새 할 일 입력 줄에 `DatePicker`를 추가하고 `SelectedDate`를 `NewTodoDueDate`에 바인딩한다.
5. 편집 상태로 `EditTodoDueDate`를 추가한다.
6. `StartEditCommand`에서 기존 할 일의 `DueDate`를 `EditTodoDueDate`로 복사한다.
7. `SaveEditCommand`에서 `EditTodoDueDate`를 원본 `TodoItem.DueDate`에 반영한다.
8. 저장, 취소, 삭제, 완료 항목 삭제, 전체 삭제 후 편집 날짜 상태가 남지 않도록 `EditTodoDueDate`를 함께 초기화한다.
9. 편집 패널에도 `DatePicker`를 추가하고 `SelectedDate`를 `EditTodoDueDate`에 바인딩한다.
10. `TodoSortOption`에 `DueDateAscending`을 추가하고 정렬 콤보박스에 `마감일순`을 추가한다.
11. `TodoItem.HasDueDate` 계산 속성으로 마감일이 없는 항목을 구분한다.
12. 마감일순 정렬에서 `HasDueDate` 내림차순, `DueDate` 오름차순, `CreatedAt` 내림차순을 함께 사용한다.
13. ViewModel 테스트에서 새 할 일 마감일 저장, 날짜 초기화, 편집 시 마감일 복사와 저장, 마감일순 정렬을 검증한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 새 할 일을 추가할 때 마감일을 선택할 수 있다.
- 마감일을 선택하지 않아도 기존처럼 할 일을 추가할 수 있다.
- 할 일 목록에는 마감일이 있을 때만 `마감일: yyyy-MM-dd`가 표시된다.
- 수정 화면에서 기존 마감일을 확인하고 변경하거나 비울 수 있다.
- 편집을 취소하거나 편집 중인 항목이 삭제되면 편집 날짜 상태도 함께 사라진다.
- `마감일순`을 선택하면 마감일 있는 항목이 먼저 나오고, 날짜가 빠른 순서로 정렬된다.
- 마감일이 없는 항목은 마감일순 정렬에서 아래쪽에 배치된다.
- 같은 마감일끼리는 최근 작성 항목이 먼저 표시된다.
- 전체 테스트 통과로 날짜 입력, 편집, 정렬 흐름이 함께 유지됨을 확인했다.

이번 단계에서 주의할 점도 확인했습니다. WPF의 `DatePicker.SelectedDate`는 `DateTime?` 타입과 잘 맞습니다. 그래서 모든 할 일에 마감일이 필수라면 `DateTime`을 쓸 수도 있지만, 이번 앱처럼 마감일이 선택 사항이면 ViewModel과 Model 모두 `DateTime?`로 두는 편이 자연스럽습니다.

또 하나의 포인트는 `null` 정렬입니다. `DueDate`만 오름차순으로 정렬하면 마감일이 없는 항목이 먼저 보일 수 있습니다. 그래서 `HasDueDate` 같은 정렬용 계산 속성을 추가해 “마감일 있음” 그룹을 먼저 만들고, 그 안에서 실제 날짜 정렬을 적용했습니다.

핵심은 `DatePicker` 하나를 붙이는 작업도 ViewModel 입력 상태, 모델 저장값, 편집 상태, 정렬 규칙, 테스트까지 함께 이어진다는 점입니다. MVVM에서는 화면 요소가 늘어날 때 그 값이 어디서 시작해 어디에 저장되고 언제 초기화되는지 추적하는 습관이 중요합니다.

## 서비스 테스트 심화 학습 메모

서비스 테스트 심화 단계에서는 ViewModel 밖에 있는 JSON 저장 서비스 자체를 테스트했습니다. 이전 ViewModel 테스트가 fake service로 화면 로직을 검증했다면, 이번 단계에서는 실제 파일 입출력을 임시 경로에 격리해서 저장 계층의 동작을 확인했습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. `TodoStorageService`에 파일 경로를 받는 생성자를 추가해 테스트에서 저장 위치를 바꿀 수 있게 한다.
2. 기본 생성자는 기존처럼 사용자 로컬 앱 데이터 폴더의 `TodoWpf\todos.json` 경로를 사용한다.
3. `TodoStorageService.Load()`는 파일이 없거나 비어 있거나 JSON이 깨졌을 때 빈 목록을 반환한다.
4. `TodoStorageService.Save()`는 저장 폴더가 없으면 먼저 폴더를 만든 뒤 JSON 파일을 쓴다.
5. `AppSettingsService`에도 설정 파일 경로를 받는 생성자를 추가해 테스트에서 실제 사용자 설정 파일을 건드리지 않게 한다.
6. `AppSettingsService.Load()`는 파일이 없거나 JSON이 깨졌을 때 기본 설정을 반환한다.
7. `TestFileHelper`로 임시 폴더와 파일 경로 생성 로직을 공통화한다.
8. 저장된 JSON에 줄바꿈과 들여쓰기가 포함되는지 확인해 사람이 읽기 쉬운 저장 형식도 테스트한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 저장 파일이 없어도 앱은 빈 할 일 목록이나 기본 설정으로 시작할 수 있다.
- 빈 할 일 JSON 파일이나 깨진 할 일 JSON 파일을 읽어도 앱이 중단되지 않는다.
- 깨진 설정 JSON 파일은 기본 설정으로 복구된다.
- 저장 시 대상 폴더가 없으면 자동으로 생성된다.
- 저장 결과는 들여쓰기된 JSON 형식으로 기록된다.
- 서비스 테스트는 실제 파일을 사용하지만, 사용자 데이터 폴더가 아니라 테스트용 임시 경로만 사용한다.
...
마감일 필터 단계에서는 기존 완료/미완료 필터와 검색 조건에 날짜 조건을 추가했습니다. 원본 목록인 `Todos`는 그대로 두고, `TodosView.Filter` 안에서 “지금 화면에 보여줄 항목인지”만 판단하는 구조는 그대로 유지했습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. `TodoDueDateFilter` enum을 만들어 `All`, `Today`, `ThisWeek`, `Overdue`, `NoDueDate` 값을 정의한다.
2. `MainWindowViewModel`에 `SelectedDueDateFilter` 속성을 추가해 현재 선택한 마감일 필터를 관리한다.
3. `OnSelectedDueDateFilterChanged()`에서 `TodosView.Refresh()`를 호출해 선택 변경 즉시 목록을 다시 계산한다.
4. `FilterTodo()`에서 상태 필터를 먼저 확인하고, 그 다음 `MatchesDueDateFilter()`로 날짜 조건을 확인한다.
5. 검색어 조건은 마지막에 적용해 상태 필터, 마감일 필터, 검색이 함께 동작하도록 만든다.
6. `Today`는 `DueDate.Date == DateTime.Today`로 오늘 날짜만 비교한다.
7. `ThisWeek`는 월요일부터 다음 주 월요일 전까지를 이번 주 범위로 계산한다.
8. `Overdue`는 `DueDate.Date < DateTime.Today`로 오늘보다 이전 마감일을 찾는다.
9. `NoDueDate`는 `DueDate`가 `null`인 항목만 표시한다.
10. `MainWindow.xaml`에 마감일 필터 `ComboBox`를 추가하고 enum 값을 `ComboBoxItem.Tag`에 연결한다.
11. ViewModel 테스트에서 오늘, 이번 주, 기한 지남, 마감일 없음, 상태 필터와 검색어 조합을 검증한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- `오늘`을 선택하면 오늘 마감인 항목만 표시된다.
- `이번 주`를 선택하면 이번 주 월요일부터 일요일까지의 마감 항목이 표시된다.
- `기한 지남`을 선택하면 오늘보다 이전 날짜가 마감일인 항목이 표시된다.
- `마감일 없음`을 선택하면 `DueDate`가 없는 항목만 표시된다.
- 상태 필터와 마감일 필터와 검색어가 동시에 적용된다.
- 전체 테스트 통과로 날짜 조건이 기존 필터, 검색, 정렬 흐름을 깨지 않음을 확인했다.

핵심은 필터 조건이 늘어나도 원본 컬렉션을 직접 바꾸지 않는 것입니다. `Todos`에는 전체 데이터가 남아 있고, `TodosView`가 상태, 마감일, 검색어 조건을 조합해 현재 화면에 보일 항목만 계산합니다. 이렇게 하면 여러 필터를 동시에 적용해도 데이터가 사라지거나 순서가 꼬이지 않습니다.

## UI 품질 개선 학습 메모

UI 품질 개선 단계에서는 새 기능을 추가하기보다, 기능이 늘어나며 복잡해진 화면을 더 읽기 좋게 정리했습니다. 입력, 필터, 정렬, 검색, 편집, 목록 관리 동작이 한 화면에 함께 있으므로, 각 영역의 역할이 자연스럽게 구분되도록 XAML 배치와 공통 스타일을 다듬었습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. 창 크기를 넓히고 높여 날짜 입력, 필터, 정렬, 편집 영역이 답답하게 보이지 않도록 한다.
2. 상태 필터 앞에 `상태`, 정렬 콤보박스 앞에 `정렬`, 마감일 필터 앞에 `마감일` 라벨을 둔다.
3. `완료 항목 삭제`와 `전체 삭제` 버튼은 필터/정렬과 섞이지 않도록 오른쪽 목록 관리 영역으로 배치한다.
4. 검색 영역과 편집 영역의 위쪽 여백을 조금 늘려 화면 덩어리가 서로 붙어 보이지 않게 한다.
5. 반복되는 `ComboBox` 색상, 테두리, 정렬 속성을 `InputComboBoxStyle`로 분리한다.
6. 반복되는 `DatePicker` 색상, 테두리, 정렬 속성을 `InputDatePickerStyle`로 분리한다.
7. 하단 안내 문구의 고정 `Gray` 색상을 제거하고 `HelpTextBlockStyle`에서 `SecondaryTextBrush`를 사용한다.
8. 어두운 테마에서 WPF 기본 `ComboBox` 닫힌 배경과 글자색이 충돌하지 않도록 `ComboBoxForegroundBrush`를 별도로 둔다.
9. XAML 파일을 수정한 뒤 줄 끝을 `CRLF`로 맞춰 편집기 경고를 줄인다.
10. 전체 테스트를 실행해 XAML 리소스 참조와 기존 ViewModel 동작이 유지되는지 확인한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 상태 필터, 정렬, 마감일 필터의 역할이 라벨로 구분된다.
- 삭제 버튼이 필터 조건이 아니라 목록 관리 동작처럼 보이도록 분리됐다.
- 메인 화면과 설정 화면의 `ComboBox`가 같은 공통 스타일을 사용한다.
- 새 할 일 입력과 수정 영역의 `DatePicker`가 같은 공통 스타일을 사용한다.
- 하단 안내 문구가 밝은 테마와 어두운 테마의 보조 텍스트 색상을 따른다.
- 어두운 테마에서도 설정 화면 `ComboBox` 선택 텍스트가 읽히도록 조정했다.
- 전체 테스트 통과로 UI 리소스 정리가 기존 기능을 깨지 않음을 확인했다.

핵심은 UI 품질 개선도 단순히 예쁘게 보이게 하는 작업이 아니라, 역할을 구분하고 반복을 줄이며 테마별 가독성을 지키는 구조 정리라는 점입니다. WPF에서는 XAML이 커질수록 화면 배치와 스타일 책임을 분리해야 이후 기능을 추가해도 파일을 읽기 쉬운 상태로 유지할 수 있습니다.

## 상태 표시와 사용자 피드백 학습 메모

상태 표시와 사용자 피드백 단계에서는 새 데이터를 추가하기보다, 이미 가지고 있는 할 일 목록과 필터 상태를 사용자가 이해하기 쉬운 문구로 보여주는 흐름을 배웠습니다. 목록 데이터 자체는 `Todos`에 있고, 화면에 표시되는 결과는 `TodosView`가 계산하므로, ViewModel은 이 둘을 조합해 요약 상태를 만들어냅니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. `TotalTodoCount`, `CompletedTodoCount`, `ActiveTodoCount` 계산 속성으로 전체, 완료, 진행 중 항목 수를 표현한다.
2. `VisibleTodoCount`는 `TodosView`를 기준으로 계산해 현재 필터와 검색 조건을 통과한 항목 수를 보여준다.
3. `TodoSummaryText`, `VisibleSummaryText`, `EmptyStateMessage`처럼 화면에 바로 바인딩하기 좋은 문자열 속성을 ViewModel에 둔다.
4. `HasNoVisibleTodos` 속성으로 결과 없음 메시지를 표시할지 결정한다.
5. 할 일 추가, 삭제, 완료 상태 변경, 검색어 변경, 필터 변경, 편집 저장 후에는 `RefreshTodoFeedback()`을 호출해 계산 속성 변경 알림을 한 번에 보낸다.
6. `SaveTodos()`와 `SaveAppSettings()`가 끝난 뒤 `SaveStatusMessage`를 갱신해 저장 완료 상태를 화면에 표시한다.
7. `MainWindow.xaml`에는 목록 위 요약 영역을 추가하고, 리스트가 비어 있을 때만 빈 상태 메시지를 보여준다.
8. `EmptyStateTextBlockStyle`은 `HasNoVisibleTodos`를 `DataTrigger`로 확인해 메시지 표시 여부를 제어한다.
9. ViewModel 테스트에서 전체/완료/진행 중 개수, 표시 중 개수, 검색 결과 없음 메시지, 저장 상태 문구 갱신을 검증한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 전체, 완료, 진행 중 항목 수가 화면에 표시된다.
- 필터나 검색 조건을 바꾸면 표시 중인 항목 수가 함께 바뀐다.
- 검색어 때문에 결과가 없을 때는 `검색 결과가 없습니다.`가 표시된다.
- 필터 조건 때문에 표시할 항목이 없을 때는 `표시할 할 일이 없습니다.`가 표시된다.
- 할 일 또는 설정 저장 후 `저장됨` 상태가 갱신된다.
- 전체 테스트 통과로 상태 피드백이 기존 추가, 삭제, 완료, 검색, 필터 흐름과 함께 유지됨을 확인했다.

핵심은 “보여주기 위한 상태”도 ViewModel의 책임이라는 점입니다. WPF 화면은 `TextBlock`과 `DataTrigger`로 ViewModel 값을 표현하고, ViewModel은 원본 목록과 `ICollectionView`의 현재 결과를 바탕으로 사용자에게 필요한 피드백을 계산합니다.

## MVVM 구조 분리 학습 메모

MVVM 구조 분리 단계에서는 `MainWindowViewModel` 안에 있던 필터 판단과 정렬 규칙을 별도 서비스로 분리했습니다. ViewModel은 화면 상태와 명령 흐름을 관리하고, `TodoFilterService`와 `TodoSortService`는 순수한 규칙 계산을 담당하도록 역할을 나눴습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. `TodoFilterService`에 상태 필터, 마감일 필터, 검색어 조건을 판단하는 메서드를 둔다.
2. 여러 조건을 조합하는 `Matches()` 메서드를 만들어 ViewModel이 필터 세부 조건을 직접 알 필요가 없게 한다.
3. 날짜 필터 테스트가 현재 날짜에 흔들리지 않도록 `today` 값을 인자로 받는 overload를 둔다.
4. `TodoSortService`에 정렬 옵션별 `SortDescription` 목록 생성 규칙을 둔다.
5. `MainWindowViewModel.FilterTodo()`는 현재 선택된 필터 값과 검색어를 `TodoFilterService.Matches()`에 넘기는 역할만 한다.
6. `MainWindowViewModel.ApplySort()`는 `TodoSortService.GetSortDescriptions()` 결과를 `TodosView.SortDescriptions`에 추가하는 역할만 한다.
7. 서비스 테스트를 추가해 ViewModel을 거치지 않고 필터와 정렬 규칙을 직접 검증한다.
8. 기존 ViewModel 테스트는 유지해 분리 후에도 실제 화면 흐름이 깨지지 않았는지 확인한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 상태 필터, 마감일 필터, 검색어 조합이 기존과 동일하게 동작한다.
- 이번 주 마감일 판단을 고정 날짜로 테스트할 수 있다.
- 최신순, 미완료순, 마감일순 정렬 규칙이 서비스 테스트로 검증된다.
- `MainWindowViewModel`에서 필터와 정렬 세부 코드가 줄어들어 화면 상태 관리 역할이 더 잘 보인다.
- 전체 테스트 통과로 구조를 분리해도 기존 앱 동작이 유지됨을 확인했다.

핵심은 구조 분리가 “파일을 많이 나누는 것”이 아니라 책임을 선명하게 만드는 작업이라는 점입니다. ViewModel은 사용자가 선택한 상태를 알고, 서비스는 그 상태를 기준으로 어떤 항목을 보여줄지와 어떤 순서로 보여줄지를 계산합니다.

## ViewModel 책임 분리 2차 학습 메모

ViewModel 책임 분리 2차 단계에서는 `MainWindowViewModel`에 남아 있던 제목 입력 검증 규칙과 상태 피드백 문구 생성 규칙을 별도 서비스로 분리했습니다. ViewModel은 여전히 화면 상태 변경 시점과 명령 실행 흐름을 담당하지만, 문자열을 어떻게 만들지와 제목이 유효한지 판단하는 세부 규칙은 서비스로 이동했습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. `TodoTitleValidationService`를 만들어 제목 공백 제거, 빈 제목 검사, 최대 길이 검사, 오류 메시지 생성을 담당하게 한다.
2. `MainWindowViewModel`의 `CanAddTodo()`, `CanSaveEdit()`, `AddTodo()`, `SaveEdit()`은 제목 검증 서비스를 호출하도록 변경한다.
3. `TodoFeedbackService`를 만들어 항목 개수 요약, 표시 중 개수 문구, 빈 상태 메시지, 저장 상태 문구 생성을 담당하게 한다.
4. `TodoSummaryText`, `VisibleSummaryText`, `EmptyStateMessage`, `SaveStatusMessage`는 ViewModel에 남겨 XAML 바인딩 지점은 바꾸지 않는다.
5. ViewModel의 `RefreshTodoFeedback()`은 그대로 두어 계산 속성 변경 알림 책임은 ViewModel에 남긴다.
6. 새 서비스 테스트를 추가해 제목 검증 규칙과 피드백 문구 생성 규칙을 ViewModel 없이 직접 검증한다.
7. 기존 ViewModel 테스트도 유지해 실제 화면 흐름에서 추가, 수정, 검색 결과 없음, 저장 상태 표시가 계속 동작하는지 확인한다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 제목 앞뒤 공백 제거 규칙이 기존과 동일하게 유지된다.
- 빈 제목과 100자 초과 제목은 계속 오류 메시지를 반환한다.
- 전체, 완료, 진행 중, 표시 중 항목 수 문구가 기존과 동일하게 생성된다.
- 검색어가 있을 때와 없을 때 빈 상태 메시지가 다르게 생성된다.
- 저장 완료 시간 문구가 고정 시간 테스트로 검증된다.
- 전체 테스트 통과로 ViewModel 책임을 더 나눠도 기존 앱 동작이 유지됨을 확인했다.

핵심은 모든 코드를 무조건 서비스로 빼는 것이 아니라, “순수 계산 규칙”과 “화면 상태 변경 타이밍”을 구분하는 것입니다. 제목 검증과 문구 생성은 같은 입력이면 같은 결과를 내는 규칙이므로 서비스 테스트에 잘 맞고, `OnPropertyChanged` 호출이나 명령 실행 순서는 여전히 ViewModel이 맡는 편이 자연스럽습니다.

## 릴리스 전 점검 루틴 학습 메모

릴리스 전 점검 루틴 단계에서는 새 기능을 추가하지 않고, 지금까지 만든 WPF 학습 앱을 배포 전에 확인할 수 있는 반복 가능한 절차로 정리했습니다. 작은 학습 앱이라도 빌드, 테스트, 게시, 실행 확인, 문서 점검을 같은 순서로 반복하면 변경 후 안정성을 확인하는 습관을 만들 수 있습니다.

이번 구현에서 배운 흐름은 다음과 같습니다.

1. `git status`로 의도한 변경과 의도하지 않은 변경을 먼저 구분한다.
2. `dotnet test`로 ViewModel과 Service 단위 테스트가 모두 통과하는지 확인한다.
3. `dotnet build --configuration Release`로 Release 구성 빌드가 가능한지 확인한다.
4. `dotnet publish --configuration Release`로 실제 배포 폴더를 생성한다.
5. 게시된 `TodoWpf.exe`를 실행해 추가, 수정, 삭제, 필터, 검색, 정렬, 설정 저장, 테마 적용을 수동으로 확인한다.
6. `docs/release-checklist.md`에 반복 가능한 체크리스트를 남긴다.
7. README에는 진행하지 않은 학습 후보를 별도 보류 항목으로 남긴다.

이번 단계에서 확인한 주요 동작은 다음과 같습니다.

- 릴리스 전 점검 순서가 별도 문서로 정리됐다.
- README에서 릴리스 체크리스트 문서로 이동할 수 있다.
- 진행하지 않은 학습 후보가 README의 `보류 학습 후보` 섹션에 남았다.
- 테스트, Release 빌드, 게시 명령을 통해 현재 프로젝트가 마무리 가능한 상태인지 확인했다.

핵심은 릴리스가 “마지막에 한 번 실행해 보는 일”이 아니라, 변경 사항을 정리하고 같은 순서로 확인하는 루틴이라는 점입니다. 학습 앱에서도 이 습관을 들이면 이후 더 큰 프로젝트에서 빌드와 테스트, 문서 상태를 함께 관리하기 쉬워집니다.
...
28. 마감일 필터: 오늘, 이번 주, 기한 지남, 마감일 없음 보기
29. UI 품질 개선: 입력 컨트롤 스타일과 화면 제어 영역 정리
30. 상태 표시와 사용자 피드백: 저장됨, 검색 결과 없음, 완료된 항목 수 표시
31. MVVM 구조 분리: 필터와 정렬 판단 로직을 서비스로 분리
32. ViewModel 책임 분리 2차: 입력 검증과 상태 피드백 계산을 서비스로 분리
33. 릴리스 전 점검 루틴: 빌드, 테스트, 게시, 실행 확인, 문서 점검

## 보류 학습 후보

1. UI 품질 개선 2차: 목록 빈 상태, 버튼 배치, 항목 템플릿을 더 세밀하게 다듬기
2. 카테고리 또는 우선순위 추가: enum, ComboBox 바인딩, 저장, 필터 확장 복습
3. 상태 표시 개선: 저장 중, 저장 실패 같은 상태를 사용자에게 보여주기
4. 검색/필터 프리셋: 자주 쓰는 필터 조합 저장과 빠른 적용

이번 WPF MVVM 할 일 앱 학습 흐름은 릴리스 전 점검 루틴까지 정리한 상태로 마무리합니다. 이후 다시 이어간다면 보류 학습 후보 중 하나를 선택해 같은 방식으로 작은 단위 구현, 테스트, 문서 갱신 순서로 진행하면 됩니다.
