...
1. 서비스 테스트 심화 2차: 기존 JSON 호환성, 알 수 없는 enum 값, 파일 접근 실패 다루기
2. UI 품질 개선 2차: 목록 빈 상태, 버튼 배치, 항목 템플릿을 더 세밀하게 다듬기
3. ViewModel 책임 분리 2차: 입력 검증과 상태 피드백 계산을 별도 helper로 분리
